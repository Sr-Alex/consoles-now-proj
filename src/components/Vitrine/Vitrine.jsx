...
function Vitrine({ products }) {
  ...
  const scroll = (to = 0) => {
    wrapper.current.scrollTo(wrapper.scrollLeft + 300, 0)
    console.log(to);
  ...
  return (
    <section className="relative">
      ...
      <button className="absolute left-4 bottom-1/2 translate-y-1/2 navArrowButton flex items-center justify-center"
      onClick={() => scroll(-300)}>
        <ArrowIcon className="w-6 h-6 fill-proj-darkGray -scale-x-100" />
      </button>
      <button className="absolute right-4 bottom-1/2 translate-y-1/2 navArrowButton flex items-center justify-center"
      onClick={() => scroll(300)}>
